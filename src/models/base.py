...
class BaseCVKAN(nn.Module, ABC):
    ...
    def _build_layers(self, aggregation: nn.Module | None = None) -> nn.ModuleList:
        """
            ...
            if self.use_multi_head:
                ...
                if aggregation is not None:
                    layer = PolarizingBlock(
                        self.d_complex, self.kan_hidden, aggregation=aggregation
                    )
                else:
    ...
    def _apply_layers(self, z: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        """
    ...
    def _pool(self, z: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if self.pooling_strategy == "mean":
            ...
            return torch.gather(z, 1, max_idx.unsqueeze(-1).expand(-1, -1, self.d_complex)).squeeze(
                1
            )
