...
class Complex2DPositionalEncoding(nn.Module):
    ...
    def __init__(
        ...
        h_div_term = torch.exp(torch.arange(0, d_h, 1) * (-math.log(base) / d_h))
        ...
        w_div_term = torch.exp(torch.arange(0, d_w, 1) * (-math.log(base) / d_w))
...
class LearnableComplexPositionalEncoding(nn.Module):
    ...
    def __init__(
        ...
        self.log_magnitudes = nn.Parameter(torch.zeros(max_len, d_complex))
