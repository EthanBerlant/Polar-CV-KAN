...
def run_config(config, train_loader, val_loader, vocab_size, device, args):
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss, train_acc = train_epoch(
            model, train_loader, optimizer, device, diversity_weight=args.diversity_weight
        )
...
def main():
    ...
    parser.add_argument(
        "--configs", type=int, default=None, help="Limit number of configs to test (for debugging)"
    )
    ...
    for r in results:
        print(
            f"{r['n_layers']:>6} {r['d_complex']:>6} {r['kan_hidden']:>6} "
