...
print("=" * 90)
print(f"{'Config':<20} | {'Epochs':<6} | {'Train MSE':<10} | {'Val MSE':<10} | {'Test MSE':<10} | {'Status':<12}")
print("-" * 90)
...
for run_dir in sorted((results_dir / 'timeseries').iterdir()):
    ...
    print(f"{config_str:<20} | {epochs:<6} | {train_mse:<10.4f} | {val_mse:<10.4f} | {test_mse:<10.4f} | {status:<12}")

print()
print("=" * 90)
print("IMAGE CV-KAN - Training Curve Analysis")
print("=" * 90)
print(f"{'Config':<20} | {'Epochs':<6} | {'Train Acc':<10} | {'Val Acc':<10} | {'Test Acc':<10} | {'Status':<12}")
print("-" * 90)
...
for run_dir in sorted((results_dir / 'image').iterdir()):
    ...
    config_str = f"d{cfg.get('d_complex', '?')} L{cfg.get('n_layers', '?')} s{cfg.get('seed', '?')}"
    print(f"{config_str:<20} | {epochs:<6} | {train_acc:<10.2f} | {val_acc:<10.2f} | {test_acc:<10.2f} | {status:<12}")
