...
def plot_layer_magnitudes(intermediates, save_path):
    ...
    for i, Z in enumerate(intermediates):
        ...
        axes[i].axvline(mags.mean(), color='red', linestyle='--', 
                        label=f'μ={mags.mean():.2f}')
...
def plot_dimension_correlation(Z, save_path):
    ...
    ax.text(0.02, 0.98, f'Mean |corr|: {mean_corr:.3f}',
...
def plot_word_magnitudes(words, magnitudes, label, pred, save_path):
    ...
    for i, (word, mag, color) in enumerate(zip(words, mags, colors)):
...
def main():
    ...
    parser.add_argument('--checkpoint', type=str, default='outputs/sst2/best.pt',
                        help='Path to checkpoint')
    parser.add_argument('--output_dir', type=str, default='visualizations/diagnostics',
    ...
    plot_layer_phases(outputs['intermediates'], 
                      os.path.join(args.output_dir, 'phase_distributions.png'))
    
    # 2. Magnitude distributions
    plot_layer_magnitudes(outputs['intermediates'],
                          os.path.join(args.output_dir, 'magnitude_distributions.png'))
    
    # 3. Dimension correlation
    Z_final = outputs['intermediates'][-1]
    plot_dimension_correlation(Z_final,
                               os.path.join(args.output_dir, 'dimension_correlation.png'))
